...
def _fmt_priveleged_info(privileged_groups, unprivileged_groups):
    privileged_info = {}
    for name, group in zip(["privileged_groups", "unprivileged_groups"], [privileged_groups, unprivileged_groups]):
...
def _format_aif360_to_sklearn(aif360_mat):
    return np.array([[aif360_mat["TN"], aif360_mat["FP"]], [aif360_mat["FN"], aif360_mat["TP"]]])
